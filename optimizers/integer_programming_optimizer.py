...
class IntegerProgrammingOptimizer(WeightsOptimizer):
    """
    Integer Programming-based implementation of WeightsOptimizer using binary variables 
    ...
    def __init__(self, data: pd.DataFrame, items: List[str], version: str, time_limit: int = 600,
                 lambda_reg: float = 50, name: Optional[str] = None):
        """
        ...
        Args:
            ...
            lambda_reg (float): Regularization strength parameter. Default is 50.
            name (Optional[str]): Custom name for the optimizer. If None, a default name is generated.
        """
        default_name = f"Integer Programming, Lambda={lambda_reg}"
        super().__init__(name=name if name else default_name, data=data, items=items, version=version)
        self.time_limit = time_limit  # Time limit in seconds
        self.lambda_reg = lambda_reg
        self.model_path = os.path.join(self.cache_dir, f'{self.version}_ip_lambda_{self.lambda_reg}.mps')
        self.solution_path = os.path.join(self.cache_dir, f'{self.version}_ip_lambda_{self.lambda_reg}.sol')
    ...
    def calc_weights(self) -> Tuple[np.ndarray, str]:
        ...
        try:
            ...
            if os.path.exists(self.model_path):
                ...
                v = [model.addVar(lb=0, ub=100, vtype=gp.GRB.INTEGER, name=f"v[{i}]") for i in range(n)]
                ...
                for i in range(len(A)):
                    ...
                    model.addConstr(A[i] @ v >= 1 - (1 - is_positive[i]) * M, name=f"positive_constraint[{i}]")
                    model.addConstr(A[i] @ v <= M * is_positive[i], name=f"positive_upper_bound[{i}]")
                    model.addConstr(A[i] @ v <= -1 + (1 - is_negative[i]) * M, name=f"negative_constraint[{i}]")
